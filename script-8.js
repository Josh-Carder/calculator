...
jQuery(document).ready(function($){
    // let userCountry = (new Date()).toString().split('(')[1].split(" ")[0];
    // let userCountry = 'England';
    ...
    // console.log(countryList[userCountry])
    ...
    $('#country').on('change', function(){
        ...
        countryData = data.filter((v,i)=>{
            if(v['Country'] === country && categoryListArry.indexOf(v['Category Name']) == -1){
    ...
    function triggerEvent(){
        if(data !='' && userCountry != ''){
            ...
            if(countryList1[userCountry]){
                document.getElementById('country').value=userCountry;
                userSymbol = countryList1[userCountry];
            }else 
            if(countryList2[userCountry]){
